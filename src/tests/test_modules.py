...
class TestBase(unittest.TestCase): 
    ...
    def assertModulePasses(self, name):
        subprocess.run(['iverilog', '-o', f'/tmp/{name}_test.vvp', f'/autograder/grader/tests/{name}_test.v', '-l/autograder/grader/tests/nand.v'] + [f'-l{p}' for p in glob.glob('/autograder/source/*.v')])
